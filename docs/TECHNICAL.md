...
3. **Email sender — Resend API key (recommended, no Gmail password).** Sign up free at resend.com
   ...
   (e.g. `you@example.com`). The key is revocable and is **not** your Gmail password.
...
`dashboard/` is a small Electron app. From the repository root, run:
...
`RESEND_API_KEY`, `MAIL_TO` (`you@example.com`), `MAIL_FROM` (optional).
...
fly secrets set DISCOGS_TOKEN=... DISCOGS_USERNAME=your_discogs_username \
  RESEND_API_KEY=re_... MAIL_TO=you@example.com \
