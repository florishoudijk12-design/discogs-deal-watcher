'use strict';
/*
 * renderer.js — dashboard UI. Talks to the cloud watcher only through window.api
 * (preload IPC). When window.api is absent (e.g. a plain browser preview), it falls
 * back to DEMO data so the layout is still viewable.
 */

const hasApi = typeof window.api !== 'undefined';

const DEMO = [
  { id: 'demo1', releaseId: 249504, artist: 'Imagination', title: 'Night Dubbing', lowest: 8.5, currency: 'EUR', numForSale: 14, reference: 32, referenceSource: 'suggestion', discount: 0.73, confidence: 2, suspicious: false, url: 'https://www.discogs.com/sell/release/249504?sort=price%2Casc', ts: Date.now() - 4 * 60000, thumb: '' },
  { id: 'demo2', releaseId: 12345, artist: 'Klein & M.B.O.', title: 'Dirty Talk', lowest: 4.0, currency: 'EUR', numForSale: 3, reference: 26, referenceSource: 'trailing-median', discount: 0.85, confidence: 1, suspicious: true, url: 'https://www.discogs.com/sell/release/12345?sort=price%2Casc', ts: Date.now() - 32 * 60000, thumb: '' },
  { id: 'demo3', releaseId: 67890, artist: 'Gino Soccio', title: 'Outline', lowest: 11.0, currency: 'EUR', numForSale: 22, reference: 24, referenceSource: 'suggestion', discount: 0.54, confidence: 2, suspicious: false, url: 'https://www.discogs.com/sell/release/67890?sort=price%2Casc', ts: Date.now() - 90 * 60000, thumb: '' },
];
...
function render() {
  ...
  const wrap = $('deals');
  const empty = $('empty');
  $('resultCount').textContent = deals.length ? `${deals.length} deal${deals.length > 1 ? 's' : ''}` : '';
  if (!deals.length) {
    wrap.innerHTML = '';
    empty.classList.remove('hidden');
    empty.textContent = allDeals.length ? 'No deals match your filters.' : 'No deals yet — the watcher will fill this in as cheap copies appear.';
    return;
  }
...
function setStatus(ok, statusObj) {
  ...
  if (statusObj) {
    ...
    $('pill-deals').textContent = `${statusObj.dealsStored ?? allDeals.length} deals stored`;
    $('pill-sweep').textContent = statusObj.lastSweepAt ? `last sweep ${ago(statusObj.lastSweepAt)}` : `sweep #${statusObj.sweepCount ?? 0}`;
  }
}
...
async function refresh() {
  if (!hasApi) {
    // Browser preview: show a real deals.json if one sits next to index.html, else DEMO data.
    try {
  ...
  try {
    ...
    setStatus(false);
    $('empty').classList.remove('hidden');
    $('empty').textContent = 'Cannot reach the watcher: ' + e.message + '  — check Settings (⚙).';
    $('deals').innerHTML = '';
  }
}
...
async function openSettings() {
  const s = hasApi ? await window.api.getSettings() : { sourceType: 'server', apiBase: '', token: '', githubRepo: '', githubToken: '' };
  $('set-sourceType').value = s.sourceType || 'server';
...
async function testConnection() {
  ...
  // Save first so the IPC handlers use the entered values.
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  refresh();
  if (hasApi) setInterval(refresh, 30_000); // poll the cloud every 30s
});
